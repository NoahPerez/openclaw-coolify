...
*   **Q: "No available server" or "502 Bad Gateway" on Coolify?**
    *   **A**: Ensure your `docker-compose.yaml` has `expose: ["24819"]` (fixed in latest version).
    *   **A**: Check logs for `OpenClaw listening on 24819`. If it says `127.0.0.1`, Traefik cannot reach it. It *must* listen on `0.0.0.0` or `lan` (default).

*   **Q: I can't connect from my Mac/PC?**
    *   **A**: Use the **Public URL** generated by Coolify (e.g., `https://openclaw.my-server.com`).
    *   **A**: DO NOT try to connect to `http://<server-ip>:24819` directly unless you manually opened that port in your server's firewall (UFW/AWS Security Group). The default setup uses reverse proxying for security.
